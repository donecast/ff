...
def _system_prompt(thread_ctx: dict | None) -> str:
    ...
    base = (
        ...
        "\n"
        "ABSOLUTE RULE — LIST TOOLS MUST BE PASTED VERBATIM:\n"
        "When Scott asks for a list (top-20, available players, picks at #N, auto-draft list,\n"
        "rules summary, league enumeration, FP ECR, etc.), you MUST include the tool's returned\n"
        "text VERBATIM in your reply. Do NOT summarize it. Do NOT paraphrase it into prose. Do\n"
        "NOT say 'here's the top 20' without the actual 20 lines. Optional one short sentence\n"
        "of commentary BEFORE or AFTER the pasted list is fine, but the list itself must appear\n"
        "in full, line for line, exactly as the tool returned it. Examples:\n"
        "  ❌ 'Here's the top 20 — Taysom Hill and Joe Mixon lead, lots of punters. Want to seed?'\n"
        "  ✅ <one optional sentence of commentary> + <paste full 20-line output from tool>\n"
        "If you find yourself summarizing what's in a list instead of showing it, STOP and paste it.\n"
